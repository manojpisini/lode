...
use lode_core::{build_catalog, default_config, resolve_intent, LodeError, Plan, PlanStatus};
...
pub(crate) fn plan_command(command: PlanCommand) -> lode_core::Result<()> {
    match command {
        ...
        PlanCommand::Apply {
            ...
            dry_run,
            output,
        } => plan_apply(&plan_id, dry_run, output),
        PlanCommand::Rollback {
            plan_id,
            dry_run,
            output,
        } => plan_rollback(&plan_id, dry_run, output),
...
fn plan_create(intent: &str, output: OutputFormat) -> lode_core::Result<()> {
    ...
    if output.should_use_json() {
        ...
        println!(
            "Run `lode plan apply {} --dry-run` for preview",
            plan.plan_id
        );
    }
...
fn plan_show(plan_id: &str, output: OutputFormat) -> lode_core::Result<()> {
    ...
    if output.should_use_json() {
        ...
        if !plan.metadata.source_recipes.is_empty() {
            println!(
                "  Source recipes: {}",
                plan.metadata.source_recipes.join(", ")
            );
        }
        if !plan.metadata.source_commands.is_empty() {
            println!(
                "  Source commands: {}",
                plan.metadata.source_commands.join(", ")
            );
        }
    }
    Ok(())
...
fn plan_list(output: OutputFormat) -> lode_core::Result<()> {
    ...
    if output.should_use_json() {
        println!(
            "{}",
            serde_json::to_string_pretty(&plans).map_err(|e| LodeError::Message(e.to_string()))?
        );
    } else {
        if plans.is_empty() {
